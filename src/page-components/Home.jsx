...
// Build YouTube embed src — simple stable, no IFrame API (enablejsapi=0)
function buildHeroYTSrc(videoId) {
  return (
    `https://www.youtube.com/embed/${videoId}` +
    `?autoplay=1&mute=1&loop=1&playlist=${videoId}` +
    `&controls=0&showinfo=0&rel=0&modestbranding=1` +
    `&iv_load_policy=3&disablekb=1&playsinline=1&enablejsapi=0`
  );
}

// ─── Hero Section ─────────────────────────────────────────────────────────────
// Priority: A) heroVideoMp4Url (native <video>) → B) heroVideoUrl (YouTube) → C) fallback image
function Hero() {
  const { t, isRTL } = useLanguage();
  const { company, primaryPhone } = useAdmin();

  const mp4Url    = company?.heroVideoMp4Url?.trim() || '';
  ...
  // ── Dark cover state — hides YouTube loading UI for 2200ms then fades out ──
  const [coverVisible, setCoverVisible] = useState(true);
  const [coverFading, setCoverFading] = useState(false);
  const coverTimerRef = useRef(null);

  // Reset MP4 state when URL changes
  useEffect(() => { setMp4Failed(false); }, [mp4Url]);

  // Start dark cover timer when YouTube iframe is active
  const showYT = !mp4Url || mp4Failed;
  useEffect(() => {
    if (!showYT) return;
    // Reset cover for new video
  ...
  }, [showYT, YT_ID]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden" style={{ background: '#F7F4EE' }}>
      ...
      <div className="hero-video-wrap">
        ...
        {/* C) Fallback image — always at z:0, visible when both video sources fail */}
        <div
          className="absolute inset-0 bg-cover bg-center"
          style={{ backgroundImage: `url('${IMGS.heroFallback}')`, zIndex: 0 }}
        />

        {/* A) Native MP4 video — primary source, works globally on all devices/regions.
            Shown when heroVideoMp4Url is set and hasn't errored.
            Falls back to YouTube on onError. */}
        {mp4Url && !mp4Failed && (
          <video
            key={mp4Url}
            autoPlay
            ...
            onError={() => { console.warn('[HeroVideo] MP4 failed — trying YouTube'); setMp4Failed(true); }}
            onCanPlay={() => console.info('[HeroVideo] MP4 canplay')}
            onPlaying={() => console.info('[HeroVideo] MP4 playing')}
            aria-hidden="true"
            style={{
              position: 'absolute',
              top: '50%', left: '50%',
        ...
        {/* B) YouTube iframe — simple stable embed, no IFrame API, no timers.
            Stays mounted permanently for valid YouTube IDs.
            Fallback image behind it handles the rare case of no video. */}
        {showYT && (
          <>
            <iframe
              key={YT_ID}
              src={buildHeroYTSrc(YT_ID)}
            ...
            />
            {/* Dark cover: hides YouTube loading UI for 2200ms, then fades out over 1.2s */}
            {coverVisible && (
              <div
                aria-hidden="true"
                style={{
                  ...
                  background: '#0a0a0a',
                  zIndex: 2,
                  transition: coverFading ? 'opacity 1.2s ease' : 'none',
        ...
        {/* Final light gradient overlay — above all video/cover layers (z:3) */}
        <div className="hero-gradient-overlay absolute inset-0" style={{ zIndex: 3 }} />
        <div
          className="absolute inset-0"
          style={{ background: 'linear-gradient(to right, rgba(247,244,238,0.78) 0%, rgba(247,244,238,0.25) 35%, rgba(247,244,238,0.05) 60%, transparent 78%)', zIndex: 3 }}
        />
      </div>
